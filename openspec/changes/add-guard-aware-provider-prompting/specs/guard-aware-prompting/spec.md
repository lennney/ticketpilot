# Guard-Aware Provider Prompting Specification

## ADDED Requirements

### Purpose

Extend the `OpenAICompatibleProvider.generate_draft()` prompt with guard-aware structured instructions that require inline `[chunk_id]` citation markers, safe fallback when evidence is insufficient, and forbidden promise patterns. These requirements support the claim guard's `has_uncited_claims` check and citation validator's structural checks. FakeLLMProvider is unaffected (template-based).
...
### Scenario: FakeLLMProvider is unaffected
- **WHEN** `TICKETPILOT_LLM_PROVIDER=fake`
- **THEN** `FakeLLMProvider.generate_draft()` uses template-based output
- **AND** guard-aware prompting does not apply

### Scenario: Prompt does not guarantee guard pass
- **WHEN** guard-aware prompt is used with real LLM
- **THEN** the LLM may still ignore citation instructions
- **AND** guard may still fail (guard effectiveness depends on LLM compliance)
- **AND** human review remains required

### Scenario: Prompt does not change DraftReply schema
- **WHEN** guard-aware prompting is applied
- **THEN** `DraftReply` fields remain unchanged
- **AND** `CitationValidator` and `ClaimGuard` remain unchanged
