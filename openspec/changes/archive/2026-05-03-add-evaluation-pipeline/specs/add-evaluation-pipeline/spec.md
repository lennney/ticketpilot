...
- intent_accuracy: all correct, some correct, none correct, empty lists
- risk_recall_precision: full, partial, no overlap; empty expected/actual
- severity_correctness: all correct, some wrong, all wrong
- must_human_review_accuracy: all correct, all wrong, mixed
- evidence_recall_at_k: k=1,3,5 with various overlap scenarios
- citation_validity: all valid, some invalid, empty citations
- draft_fallback_correctness: all correct, all wrong, mixed
- unsupported_claim_correctness: all correct, mixed
- human_review_readiness: all, some, none passing
- All metric return dicts have correct structure
...
- Pipe-delimited fields parse into lists
- Boolean fields parse from string
- ticket_id duplicates raise error

test_evaluation_report.py:
- build_report produces correct summary from sample results
- write_report produces valid JSON file
- Metadata includes required fields
- Limitations text present
- Empty result list produces zeroed summary

test_evaluation_comparison.py:
- Full match produces all-correct results
- Partial match produces correct partial scores
- Missing golden field raises clear error
- Pipeline error recorded in errors list
- None values handled gracefully

### Integration Tests

test_evaluation_pipeline.py:
- Load test fixture CSVs (3-5 tickets)
- Run full evaluation pipeline
- Verify report structure: metadata, summary, per_ticket
- Verify all 9 metric categories present in summary
- Verify per_ticket entries match ticket count
- Verify JSON serialization roundtrip
- Verify determinism: two consecutive runs produce identical report
- Verify failed tickets recorded in report
- Verify error handling for invalid pipeline state
