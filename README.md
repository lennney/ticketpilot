...
**项目阶段：** Phase 7（MVP 数据与评估）→ Phase 8（混合检索升级）→ Phase 9（评估驱动知识优化）→ Phase 10（排名诊断与细粒度评测）→ Phase 11（证据驱动 LLM 草稿生成，已完成）→ Phase 12（Provider 对比与作品集文档，已完成）

**评测指标：** Doc-ID Recall@10 = 91.9%（Phase 10 细粒度诊断）| Provider 对比：Fake/Real 双 Provider 25/25 成功（Phase 12）| 质量门禁：1069 单元 + 146 集成测试，87% 覆盖率
...
| 方向 | 说明 | 依赖 |
|------|------|------|
| **Phase 11 收尾** | 完成 LLM provider 实现、ClaimGuard、离线草稿评估 | 无 |
| **检索排序优化** | 基于 Phase 10 诊断结果优化 RRF 权重、query expansion | Phase 10 基线 |
