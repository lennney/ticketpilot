...
class ClaimProvenance(BaseModel):
    ...
    source_chunk_id: UUID = Field(
        description="Knowledge chunk UUID (matches FusedResult.chunk_id)"
    )
    source_doc_id: UUID = Field(
        description="Source document UUID (matches FusedResult.doc_id)"
    )
    source_doc_type: str = Field(description="Document type: faq / policy / case")
    retrieval_method: str = Field(
        description="How chunk was retrieved: keyword / vector / fused"
    )
...
class ResponseProvenance(BaseModel):
    ...
    claims: list[ClaimProvenance] = Field(
        default_factory=list, description="Provenance for each cited claim"
    )
    overall_confidence: float = Field(
        ge=0, le=1, description="Overall provenance confidence"
    )
