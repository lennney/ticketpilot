...
class ReviewDecision(BaseModel):
    ...
    provider_name: str | None = Field(
        default=None, description="LLM provider name (e.g. 'fake')"
    )
    model_name: str | None = Field(
        default=None, description="Model name used by provider"
    )
    ...
    guard_passed: bool | None = Field(
        default=None, description="Whether all claim guard checks passed"
    )
    ...
    human_review_forced: bool | None = Field(
        default=None, description="Whether human review was forced by validation/guard"
    )
    ...
    confidence_level: str | None = Field(
        default=None, description="Confidence tier: high/medium/low/critical"
    )
    confidence_retrieval: float | None = Field(
        default=None, ge=0, le=1, description="Retrieval confidence dimension"
    )
    confidence_classification: float | None = Field(
        default=None, ge=0, le=1, description="Classification confidence dimension"
    )
    confidence_citation: float | None = Field(
        default=None, ge=0, le=1, description="Citation coverage confidence dimension"
    )
    confidence_evidence_density: float | None = Field(
        default=None, ge=0, le=1, description="Evidence density confidence dimension"
    )
